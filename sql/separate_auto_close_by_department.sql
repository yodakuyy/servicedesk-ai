-- =====================================================
-- Auto Close Rules - Department Awareness Upgrade
-- =====================================================

BEGIN;

-- 1. Add company_id to auto_close_rules
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'auto_close_rules' AND column_name = 'company_id') THEN
        ALTER TABLE public.auto_close_rules ADD COLUMN company_id INT REFERENCES public.company(company_id);
    END IF;
END $$;

-- 2. Update the processing function to respect company_id
...
BEGIN
        ...
        IF v_rule.condition_type = 'status' THEN
            ...
            FOR v_ticket IN
                ...
                WHERE ts.status_name ILIKE v_rule.condition_value
                  ...
                  AND (v_rule.company_id IS NULL OR tc.company_id = v_rule.company_id)
            LOOP
                -- Close the ticket
                UPDATE tickets
                ...
                IF v_rule.notify_user AND v_ticket.requester_id IS NOT NULL THEN
                    ...
                    SELECT v_ticket.requester_id, 'Ticket Auto-Closed', 
                           'Ticket ' || v_ticket.ticket_number || ' has been automatically closed due to inactivity.',
                           'ticket_closed', 'ticket', v_ticket.id, false, NOW(), tc.company_id
                    FROM tickets t2 JOIN ticket_categories tc ON t2.category_id = tc.id WHERE t2.id = v_ticket.id;
                END IF;

                -- Notify agent if configured
                IF v_rule.notify_agent AND v_ticket.assigned_to IS NOT NULL THEN
                    INSERT INTO notifications (user_id, title, message, type, reference_type, reference_id, is_read, created_at, company_id)
                    SELECT v_ticket.assigned_to, 'Ticket Auto-Closed', 
        ...
        ELSIF v_rule.condition_type = 'user_confirmed' THEN
             ...
             FOR v_ticket IN
                ...
                WHERE ts.status_name ILIKE 'Resolved'
                  ...
                  -- User confirmed usually closes much faster or immediately
                  AND t.updated_at <= v_cutoff_time
                  AND (v_rule.company_id IS NULL OR tc.company_id = v_rule.company_id)
            LOOP
                UPDATE tickets
    ...
    v_result := jsonb_build_object(
        'success', true,
        'processed', v_total_processed, -- This logic needs better processed tracking if wanted, but for now we focus on closed
