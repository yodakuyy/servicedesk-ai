-- ==============================================================================
-- IMPLEMENTASI ROUND ROBIN TICKET ASSIGNMENT
-- ==============================================================================
...
DECLARE
    next_agent_id UUID;
BEGIN
    -- Logika: Cari user di grup tersebut yang paling lama tidak menerima tiket (least recently assigned)
    -- Kita abaikan user dengan Role Requester (role_id = 4)
    
    SELECT ug.user_id INTO next_agent_id
    FROM public.user_groups ug
    JOIN public.profiles p ON ug.user_id = p.id
    LEFT JOIN (
        -- Hitung kapan terakhir kali agent ini menerima tiket di grup ini
    ...
    AND p.role_id != 4 -- Bukan Requester
...
-- 5. Catatan: Pastikan tabel user_groups menggunakan 'group_id' yang sama 
-- dengan assignment_group_id di tabel tickets.
